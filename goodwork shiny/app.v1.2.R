...
ui <- bootstrapPage(
  ...
  absolutePanel(top = 10, right = 10, width = 300,
                sidebarPanel(width = 17,
                  ...
                  tags$script(type="text/javascript",
                              HTML("VK.Widgets.Auth('vk_auth',
                           {authUrl: 'http://r.piterdata.ninja/p/6230/'});"))
                )))
    ...
    tags$script(HTML('
      ...
      output$map <- renderLeaflet({
        ...
        if (input$recommand == 0){
          #нужны цвета, зависящие от оценки шавы
        ...
        #красим рекоменды
          ...
          shavas$reco = 1
          recorots <- left_join(egarots, shavas, by = "idshava")
          
          #цвета
          getColor <- function(recorots) {
            sapply(recorots$reco, function(reco) {
              if(reco == 1) {
                "green"
              } 
              else {
          ...
          output$map <- renderLeaflet({
            leaflet(recorots) %>%
              addProviderTiles(providers$Stamen.TonerLite,
                               options = providerTileOptions(noWrap = TRUE)) %>%
              addAwesomeMarkers(~coord1, ~coord2, layerId = c(recorots$idshava), icon = icons, popup = "<div id=\"popup\" style='min-width:250px;max-height:500px'
                     class=\"shiny-html-output\"></div>")})
        }
        })
