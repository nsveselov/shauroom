...
server = function(input, output, session){
  ...
  # print(query$uid[1])
  # getURL(paste('https://api.vk.com/method/users.getSubscriptions?user..', query$uid[1], 
  ...
  output$hummary <- renderText({
    ...
    isolate(getURL(paste('https://api.vk.com/method/users.getSubscriptions?user..', query$uid[1], 
  ...
  # #иконы
  # Icons <- icons(
  # iconUrl = "https://lut.im/XdivheyfQy/IewImMK9IMClPtCV.png", # p
  # iconWidth = 19, iconHeight = 37)
  # 
  # 
  # leaflet(egarots) %>%
  ...
  # addMarkers(~coord1, ~coord2, layerId = c(egarots$idshava), icon = Icons,
  ...
  observeEvent(input$recommand, {
    ...
    write.csv(shavas, "shavas.csv", row.names = F)
    egarots$reco = 0
    ...
    Icons <- icons(
      iconUrl = ifelse(egarots$reco > 0.5,
                       "https://lut.im/0JP9A77h15/hF0ECvlrtQPT6hig.png", # b
                       "https://lut.im/XdivheyfQy/IewImMK9IMClPtCV.png" # p
      ))
    
    leafletProxy("map") %>%
      setView(lng = 30.307184, lat = 59.944156, zoom = 10) %>% 
      clearMarkers() %>%
      addMarkers(egarots$coord1, egarots$coord2, layerId = c(egarots$idshava), icon = Icons,
                 popup = "<div id=\"popup\" style='min-width:250px;max-height:500px'
                 class=\"shiny-html-output\"></div>")
  })
  ...
  observeEvent(input$top10, {
    ...
    leafletProxy("map") %>%
      setView(lng = 30.307184, lat = 59.944156, zoom = 10) %>% 
      clearMarkers() %>%
      addMarkers(top$coord1, top$coord2, layerId
                 = c(top$idshava), icon = Icons,
                 popup = "<div id=\"popup\" style='min-width:250px;max-height:500px'
                 class=\"shiny-html-output\"></div>")
  })
  ...
  observeEvent(input$top30, {
    ...
    leafletProxy("map") %>%
      setView(lng = 30.307184, lat = 59.944156, zoom = 10) %>% 
      clearMarkers() %>%
      addMarkers(top$coord1, top$coord2, layerId = c(top$idshava), icon = Icons,
                 popup = "<div id=\"popup\" style='min-width:250px;max-height:500px'
                 class=\"shiny-html-output\"></div>")
  })
...
class=\"shiny-html-output\"></div>")
      ...
      shavasonly<-read.csv("shavas.csv")
      
      Icons <- icons(
        iconUrl = "https://lut.im/0JP9A77h15/hF0ECvlrtQPT6hig.png", # p
        iconWidth = 19, iconHeight = 37)
      
      leafletProxy("map") %>%
        setView(lng = 30.307184, lat = 59.944156, zoom = 10) %>% 
        clearMarkers() %>%
        addMarkers(shavasonly$coord1, shavasonly$coord2, layerId = c(shavasonly$idshava), icon = Icons,
                   popup = "<div id=\"popup\" style='min-width:250px;max-height:500px'
class=\"shiny-html-output\"></div>")
    }})
